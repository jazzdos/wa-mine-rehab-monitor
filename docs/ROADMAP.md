...
| 4 | **Batch F context**: DBCA-060 fire overlap (three-state, never inferred absence) and SILO climate joined to trajectories. `fetch-silo` (owner-run, not yet run against the real bucket) downloads the SILO open-data grids; `build-climate-context` builds the curated climate context from those grids | Mirror declined, F1 dissolved 2026-08-29 (A10); fetch-dbca-fire + build-fire-context landed AND run live 2026-08-29 (snapshot 149,621 features verified; fire_context.parquet 404,508 rows, 1987-2025); SILO fetch still owner-run; F6 join still requires E4 | D13 §5 |
